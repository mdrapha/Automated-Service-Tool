...
# from ChatBot import ChatBot -> Essa parte deve ser utilizada com a nossa api do chat gpt
import time, sys
...
class WhatsAppBot:
    ...
    def load_conversations_list(self):
        ...
        conversations = self.driver.find_elements(By.XPATH, "//span[@title]")
        self.conversations_list = [
            conv.get_attribute("title") for conv in conversations
        ]

    def load_conversation(self):
        self.driver.get(f"https://web.whatsapp.com/send?phone={self.numero}")
        ...
        while (
            len(
                self.driver.find_elements(
                    By.XPATH,
                    r"/html/body/div[1]/div/div/div[4]/div/div[2]/div/div[2]/div[2]/button",
                )
            )
            > 0
        ):
    ...
    def read_and_respond(self):
        ...
        while True:
            ...
            elements = self.driver.find_elements(
                By.XPATH, f"//span[@data-testid='conversation-info-header-chat-title']"
            )
            Name = f"{elements[0].text}:"
            divs = self.driver.find_elements(
                By.XPATH, f"//div[contains(@data-pre-plain-text, '{Name}')]"
            )
            if divs:
                last_parent = divs[-1]
                child_elements = last_parent.find_elements(
                    By.XPATH, "./*[not(contains(@class, '_1hl2r'))]"
                )
                last_child = child_elements[-1]
                text = last_child.text.replace("\n", "")
                if text in self.messages:
                    resposta = self.Conversa.Question(text)
                    if resposta:
    ...
    def send_message(self, message):
        ...
        elemento = self.driver.find_element(
            By.XPATH,
            f'//*[@id="main"]/footer/div[1]/div/span[2]/div/div[2]/div[1]/div/div/p',
        )
        elemento.clear()
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        raise Exception(
            "Phone number and API_KEY is mandatory and must be passed as an argument."
        )
...
"""
    ...
    def run(self):
        ...
        self.driver.get(
            f"https://web.whatsapp.com/send?phone={self.numero}"
        )  # Navega até a página de conversa
        while (
            len(self.driver.find_elements(By.ID, "side")) < 1
        ):  # Aguarda até a página ser carregada completamente
            time.sleep(1)
        time.sleep(2)
        print("Sincronizando Mensagens...")
        while (
        ...
        ) > 0:  # Esperando as mensagens serem Sincronizadas
            time.sleep(1)
        time.sleep(2)
        print("Mensagens Sincronizadas")
        ...
        while True:  # Loop infinito para verificar novas mensagens
            ...
            elements = self.driver.find_elements(
                By.XPATH, f"//span[@data-testid='conversation-info-header-chat-title']"
            )  # Encontra o elemento que contem o nome da pessoa
            ...
            if divs:
                ...
                child_elements = last_parent.find_elements(
                    By.XPATH, "./*[not(contains(@class, '_1hl2r'))]"
                )  # Obtendo os elementos filhos do Div
                ...
                if (
                    text in self.messages
                ):  # Verifica se o texto esta na lista self.menssages
                    resposta = self.Conversa.Question(
                        text
                    )  # Manda a última mensagem do usuário para o método "Question" da classe "ChatBot"
                    if resposta:  # Verifica o método 'Question' retornou uma resposta
                        time.sleep(
                            2
                        )  # Aguarda 2 segundos, tornando o script mais humano
                        elemento = self.driver.find_element(
                            By.XPATH,
                            f'//*[@id="main"]/footer/div[1]/div/span[2]/div/div[2]/div[1]/div/div/p',
                        )  # Obtendo elemento responsável por mandar as mensagens
                    ...
                    self.messages.append(
                        text
                    )  # Adiciona a ultima mensagem na lista self.messages
            else:
                print("Não há mensagens do usuário.")
...
if __name__ == "__main__":
    ...
    phone_number = sys.argv[
        1
    ]  # Obtém o número de telefone do destinatário da linha de comando
